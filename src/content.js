(function () {
  ...
  function isShopify() {
    ...
    if (scripts.some((s) => /cdn\.shopify\.com/i.test(s.src))) return true;

    const links = Array.from(document.querySelectorAll("link[href]"));
    if (links.some((l) => /cdn\.shopify\.com/i.test(l.href))) return true;

    if (document.cookie && /_shopify_/i.test(document.cookie)) return true;

    return false;
  }

  function injectBanner() {
    if (document.getElementById("shopify-custom-banner")) return;


    const banner = document.createElement("div");
    banner.id = "shopify-custom-banner";
    banner.innerHTML = `
      <div class="custom-banner-inner">
        <div class="custom-banner-left">
         <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20" viewBox="0 0 48 48">
          ...
          <span class="custom-banner-text">Built with <strong>Shopify</strong></span>
        </div>
        <div class="custom-banner-right">
          <button class="custom-banner-close" aria-label="Dismiss">✕</button>
        </div>
      </div>
    `;
    ...
    style.textContent = `
      #shopify-custom-banner {
        ...
        font-size: 14px;
        color: #fff;
        animation: custom-banner-in 0.4s cubic-bezier(0.16,1,0.3,1) both;
      }
      @keyframes custom-banner-in {
      ...
      .custom-banner-left  { display: flex; align-items: center; gap: 10px; }
      .custom-banner-right { display: flex; align-items: center; gap: 12px; }
      .custom-banner-text  { font-size: 13px; color: rgba(255,255,255,0.9); }
      .custom-banner-text strong { color: #7cb342; font-weight: 700; }
      .custom-banner-close {
        background: none;
        border: none;
      ...
      .custom-banner-close:hover { color: #fff; background: rgba(255,255,255,0.1); }
    `;

    document.head.appendChild(style);
    document.body.insertBefore(banner, document.body.firstChild);
    document.documentElement.style.marginTop =
      (banner.offsetHeight || 46) + "px";

    banner.querySelector(".custom-banner-close").addEventListener("click", () => {
  ...
  function notifyBackground(detected) {
    chrome.runtime.sendMessage({ type: "SHOPIFY_DETECTION", detected });
  }

  const detected = isShopify();
  notifyBackground(detected);
